...
**对话**
- SSE 流式输出，逐字返回
- 六档回复长度（lite / quick / classic / smooth / story / epic），
  各自独立的 token 预算与 hearts 消耗
- 消息编辑、朗读（TTS）
...
- 图片生成：文生图 / 图生图（角色头像）
- 语音合成：每个角色独立音色
- 编辑资料：昵称、用户自称（角色怎么称呼你）、人设描述

**规划中（服务端接口已就绪，界面未接入）**
- 任务、成就、通知、贴纸
- 头像上传

> 上面这几项数据库表与接口都已建好，但前端没有入口 —— 用户看不到，
> 所以不列在「功能」里。这样标注是为了避免把「已规划」说成「已实现」。
